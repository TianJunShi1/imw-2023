...
canvas.onmousedown = function (event) {
  ...
  for (let i = 0; i < bubbles.length; i++) {
    if (isPointInsideBubble(mouseX, mouseY, bubbles[i])) {
      if (bubbles[i].type === "shape") {
...
function update () {
  ...
  for (let i = 0; i < bubbles.length; i++) {
    ...
    if (o.type === "text") {  //文字
      drawBubblesText(o)
    } else if (o.type === "shape") { //图形
      drawBubblesShape(o)
    }

    // 判断泡泡是否被点击或按下正确的键
    if ((o.clicked && o.type === "shape") || (o.type === "text" && o.content === keyPressed)) {
...
// 绘制泡泡里面的图形
function drawBubblesShape (o) {
  ctx.beginPath()
  if (o.content == 'circle') {
    ctx.arc(o.x, o.y, o.radius, 0, Math.PI * 2)
  } else if (o.content == 'square') {
    ctx.rect(o.x - o.radius, o.y - o.radius, o.radius * 2, o.radius * 2)
  } else if (o.content == 'rectangle') {
    ctx.rect(o.x - o.radius, o.y - o.radius / 2, o.radius * 2, o.radius)
  } else if (o.content == 'triangle') {
    ctx.moveTo(o.x, o.y - o.radius)
    ctx.lineTo(o.x + o.radius, o.y + o.radius)
    ctx.lineTo(o.x - o.radius, o.y + o.radius)
    ctx.closePath()
  } else if (o.content == 'diamond') {
    ctx.beginPath()
    ctx.moveTo(o.x, o.y - o.radius) // 上顶点
    ctx.lineTo(o.x + o.radius, o.y) // 右顶点
    ctx.lineTo(o.x, o.y + o.radius) // 下顶点
    ctx.lineTo(o.x - o.radius, o.y) // 左顶点
    ctx.closePath()
    ctx.fill()
  }
  ctx.fillStyle = o.color
...
function generateBubble (num) {
  ...
  //设置的规则是每消除一个，再生成1个，
  // 控制当随机生成的文字泡泡是1的时候，也就是生成文字泡泡，不生成图形泡泡了
  if (num != 1) {
    let c = o1.change.c += o1.c
    const shapeBubble = {  //图形
      x: rand() * (w - 60) + 30, // 随机位置，确保不会碰到边界
      y: rand() * (h - 60) + 30,
      radius: o1.radius,  //半径
      color: 'hsla(' + c + ', 100%, 50%, 1)',  //颜色
      content: getRandomShapeContent(),  //图形
      type: "shape",
      clicked: o1.clicked,  //是否被点击
      dx: (rand() * 4 - 2) * speedFactor, // x 方向的速度
      dy: (rand() * 4 - 2) * speedFactor, // y 方向的速度
...
// 随机生成图形
function getRandomShapeContent () {
